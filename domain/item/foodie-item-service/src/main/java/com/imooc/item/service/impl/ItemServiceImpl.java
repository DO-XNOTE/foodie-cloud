...
public class ItemServiceImpl implements ItemService {
    ...
    public Items queryItemById(String itemId) {
        ...
        list.add(items);
        int size = list.size();
        String s = JsonUtils.objectToJson(items);
        log.info(" size = " + size + "=========== item {} " + s);
        return items;
    }
    ...
    @Transactional(propagation = Propagation.SUPPORTS)
    @Override
    public List<ShopcartVO> queryItemsSpecIds(String specIds) {
        /** 拆分规格 */
        String[] ids = specIds.split(",");
        List<String> specIdsList = new ArrayList<>();
        /** 工具类 ：把一个元数据，天骄到目标的一个List中去 , 就是一个集合中元素，全部添加到另一个集合中去 */
        /**
         * 把 ids 全部添加到 specIdsList 中去
         */
        Collections.addAll(specIdsList,ids);
        return itemsMapperCustom.queryItemsSpecIds(specIdsList);
    }
